...
public class Graphs {
    ...
    /**
     * Computes 2D bounds of all verts in graph.
     *
     * @param verts  List of verts, linked on mGraphNext.
     * @param outBox Length-4 array to hold [xMin,yMin,xMax,yMax] on output.
     */
    ...
    /**
     * Computes 3D bounds of all verts in graph.
     *
     * @param verts  List of verts, linked on mGraphNext.
     * @param outBox Length-6 array to hold [xMin,yMin,zMin,xMax,yMax,zMax] on output.
     */
    ...
    /**
     * Randomize XY coords of 2D Verts.
     *
     * @param verts   List of verts, linked on mGraphNext.
     * @param bounds  Length-4 array holding [xMin,yMin,xMax,yMax].
     * @param optRand [optional] random number generator.
     */
    public static void randomizePositions2( Vert verts, float[] bounds, Random optRand ) {
        ...
        final float sx = bounds[2] - bounds[0];
        final float ty = bounds[1];
        final float sy = bounds[3] - bounds[1];
    ...
    /**
     * Randomize XYZ coords of 3D verts.
     *
     * @param verts   List of verts, linked on mGraphNext.
     * @param bounds  Length-6 array holding [xMin,yMin,zMin,xMax,yMax,zMax].
     * @param optRand [optional] random number generator.
     */
    ...
    /**
     * @param list  List of verts, liken on mGraphNext.
    ...
    /**
     * @param list  List of edges, linked on mGraphNext.
     * @return length of linked list.
     */
    ...
    /**
     * Coarsens a graph according to some strategy define in LayoutParams.
     * Note that the {@code src} graph no NO LONGER be valid after this method
     * is called. To get a usuable version of {@code src}, you must call {@code uncoarsen}
     * on this graph returned by this method.
     *
     * @param  params Supplies the coarsening strategy to use.
     * @param  src    Source graph. NOT USUABLE after coarsening.
     * @return Coarsened version of graph, or null if graph cannot be further coarsened.
     */
    ...
    /**
     * Reversed a graph coarsening step and refines resulting graph.
     * <p>
     * Note that the {@code src} graph no NO LONGER be valid after
     * calling this method.
     *
     * @param  params Supplies the graph refinement strategy to use.
     * @param  src    Source graph. NOT USUABLE after coarsening.
     * @return Finer version of graph, or null if none exists.
     */
    public static Graph uncoarsen( LayoutParams params, Graph src ) {
        ...
        Vert removalNonce = new Vert();
        Edge reactivateEdges = dst.mEdges;
        
        // Loop through all finer-level verts store in the dst graph and mark
        // the owners with a unique removalNonce that indicates they will be discarded.
        for( Vert v = dst.mVerts; v != null; v = v.mGraphNext ) {
            Vert owner = v.mGraphOwner;
            owner.mTempNext = removalNonce;
        }
        ...
        while( loopVert != null ) {
            ...
            if( v.mTempNext == removalNonce ) {
                ...
                while( loopEdge != null ) {
                    ...
                    if( e.other( v ).mTempNext != removalNonce ) {
                        v.addEdge( e );
                        if( e.mA == v ) {
                            dst.addEdge( e );
                        }
    ...
    /**
     * Computes estimate of graph diameter.
     * <p>
     * Overwrites mTempNext and mTempDist
     */
    ...
    /**
     * Performs several checks on the internal consistency of a graph object.
     */
